...
class HSIData:
    ...
    def __get_patch(self,  indices, patch_len, if_3d=False):
        pad_len = patch_len // 2
        padded_data = np.lib.pad(self.data,
        ...
        # idx_pos in padded_data
        idx_pos = np.unravel_index(indices, (self.__H, self.__W), 'C')
        # idx_t = self.__get_pos(indices)

        patch = np.zeros((idx_pos[0].shape[0], patch_len, patch_len, self.__B))
        for i in range(idx_pos[0].shape[0]):
            idx_row = idx_pos[0][i] + pad_len
            idx_col = idx_pos[1][i] + pad_len
            start_row, start_col = idx_row - pad_len, idx_col - pad_len
            end_row, end_col = idx_row + pad_len, idx_col + pad_len
            patch[i, :, :, :] = padded_data[start_row:end_row+1, start_col:end_col+1, :]
        new_gt = self.gt - 1
        label = new_gt[idx_pos]

        if if_3d:
            pass#patch = np.expand_dims(patch,1) #batch, 1, h,w,b
        else:
            patch = patch.transpose(0, 3, 1, 2)  # batch, B, H, W
        return patch, label

    def __to_patchloader(self, patch, label, idx, batch_size, shuffle=True):
        patch_tensor = torch.from_numpy(patch).type(torch.FloatTensor).unsqueeze(1)
        label_tensor = torch.from_numpy(label).type(torch.FloatTensor)
        idx_tensor = torch.from_numpy((np.array(idx))).type(torch.IntTensor)
        torch_dataset = Data.TensorDataset(patch_tensor, label_tensor, idx_tensor)
        patch_loader = Data.DataLoader(
    ...
    def get_dataloader(self, train_ratio, val_ratio=None, patch_len=None,
                             batch_size=None, if_3d=False, minimun=1, seed=None):
        if val_ratio==None:#only train and test
            ...
            train_patch, train_label = self.__get_patch(train_idx, patch_len, if_3d)
            train_loader = self.__to_patchloader(train_patch, train_label, train_idx, batch_size)

            test_patch, test_label = self.__get_patch(test_idx, patch_len, if_3d)
            test_loader = self.__to_patchloader(test_patch, test_label, test_idx, batch_size, False)
            ...
            val_patch, val_label = self.__get_patch(val_idx, patch_len, if_3d)
            val_loader = self.__to_patchloader(val_patch, val_label, val_idx, batch_size)
            return train_loader, test_loader, val_loader

    def get_fulldataloader(self, patch_len, batch_size, if_3d=False):
        full_idx = np.arange(self.__H*self.__W)
        full_patch, full_label = self.__get_patch(full_idx, patch_len, if_3d)
        full_loader = self.__to_patchloader(full_patch, full_label, full_idx, batch_size, False)
        return full_loader
class Toolkits:
    @staticmethod
...
class ModelTrainer:
    ...
    def train_per_epoch(self, train_loader, val_loader):
        ...
        train_labels = []
        # training phase
        for idx_batch, (inputs, targets, _) in enumerate(train_loader):
            self.net.train()
            ...
            loss.backward()
            self.optimizer.step()
            train_pred = np.append(train_pred, Toolkits.to_numpy(outputs.argmax(dim=1)))
            train_labels = np.append(train_labels, Toolkits.to_numpy(targets))
    ...
    def evaluate(self, val_loader, if_test=False):
        ...
        with torch.no_grad():
            self.net.eval()
            for idx_val_batch, (inputs_val, targets_val, _) in enumerate(val_loader):
                ...
                val_pred = np.append(val_pred, Toolkits.to_numpy(outputs_val.argmax(dim=1)))
                val_labels = np.append(val_labels, Toolkits.to_numpy(targets_val))
        if not if_test:
            val_acc = metrics.accuracy_score(val_labels, val_pred)
            return val_acc, loss_val
        else:
            return val_labels, val_pred#, val_idx.astype(int)

    def evaluate_full(self, full_loader):
        val_pred = []
        with torch.no_grad():
            self.net.eval()
            for idx_val_batch, (inputs_val, targets_val, _) in enumerate(full_loader):
                inputs_val, targets_val = inputs_val.to(self.device), targets_val.to(self.device)
                outputs_val = self.net(inputs_val)
                val_pred = np.append(val_pred, Toolkits.to_numpy(outputs_val.argmax(dim=1)))
        return val_pred
...
class Plotkits:
    @staticmethod
    def draw_classification_map(path, gt, idx, pred, colors=None, scale: float = 4.0, dpi: int = 400):
        '''
        get classification map , then save to given path
        '''
        label = gt
        idx_pos = np.unravel_index(idx, label.shape)
        label[idx_pos] = pred + 1
        #还原回0为背景
        fig, ax = plt.subplots()
    ...
    @staticmethod
    def draw_gt_map(path, label, colors=None, scale: float = 4.0, dpi: int = 400):
        '''
        get classification map , then save to given path
        '''
        fig, ax = plt.subplots()
...
if __name__ == '__main__':
    ...
    hsi = HSIData(data_name='IP')
    H, W, B = hsi.get_size()
    num_cls, num_total, count_cls = hsi.get_clsinfo()
    pass
